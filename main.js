...
Linux linux24-sim 5.15.0-sim #1 SMP PREEMPT GNU/Linux
  ...
  function brickDevice(message) {
    ...
    if (!brick) {
      ...
      brick.innerHTML = `<div style="font-size:1.2em;font-weight:800;margin-bottom:12px;">DEVICE BRICKED</div><div style="max-width:86%;text-align:center;opacity:0.95;">${message || 'The device is permanently bricked (simulated).'}<br><br>Factory restore required.</div>`;
      device.appendChild(brick);
    }
    
    // visually mark device; disable pointer events globally
    device.style.pointerEvents = 'none';
    document.body.style.pointerEvents = 'none';
    // keep overlay interactive so it blocks clicks (but not dismissible)
